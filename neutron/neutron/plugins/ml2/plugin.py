...
class Ml2Plugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    # 创建neutron.plugin.ml2.rpc.RpcCallbacks类的实例
    def _setup_rpc(self):
    ...
    # ML2Plugin初始化时，针对agent创建自己的消息队列（notify，生产者）（topics.AGENT）以便向agent发送rpc请求，
    # 同时订阅ml2 Agent消息队列的消息（consumer，消费者）（topics.PLUGIN）以便接收来自agent的rpc请求。
    # ml2 Agent初始化时，也会创建自己的消息队列（notify，生产者）（topics.PLUGIN）来向plugin发送rpc请求，
    # 同时订阅ML2Plugin消息队列的消息（consumer，消费者）（topics.AGENT）来接收来自plugin的rpc请求。
    @log_helpers.log_method_call
    def start_rpc_listeners(self):
        ...
        # 创建了dispatcher处理topic = ‘q-plugin’,
        # 用来订阅ml2 Agent消息队列的消息（consumer，消费者）（topics.PLUGIN）以便接收来自agent的rpc请求。
