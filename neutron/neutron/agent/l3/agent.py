...
class L3NATAgent(ha.AgentMixin,
    ...
    def __init__(self, host, conf=None):
        ...
        # 该self.plugin_rpc会处理neutron-server转发过来的请求，这个请求是通过service_plugins的方式处理
    ...
    # 这个函数里会去调用_process_routers_loop函数，在_process_routers_loop函数中会去创建绿色线程，执行_process_router_update函数。
    # 可以说，l3 agent调用网络设备的工作都会在_process_router_update中进行。
