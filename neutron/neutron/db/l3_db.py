...
# 继承了上面l3模块的class RouterPluginBase，因此在RouterPluginBase中定义的抽象方法就要在这里实现了
# 在上面的l3_db.py中，会将涉及router和floating ip的处理读取或者写入到数据中。
# 但是还有一些操作不仅如此，还需要通过rpc（通过调用l3_rpc_agent_api中的函数，这些操作大部分会去 调用routers_updated），
# 通知l3 agent进行处理。
# 这些需要处理的地方包括（对router，floating_ip，interface的删del、改update操作）：
#   update_router，
#   delete_router，
